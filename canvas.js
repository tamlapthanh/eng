...
(function (global) {
  const CanvasManager = (function () {
    ...
    let swipeState = { active: false, startX: 0, startY: 0, startTime: 0, fired: false };
    let lastSwipeTime = 0;
    ...
    function formatNumber(n, decimals = 6) { return Number(n.toFixed(decimals)); }
    ...
    const newScale = Math.min(maxZoom, Math.max(minZoom, oldScale + (delta > 0 ? +delta : -Math.abs(delta))));
    zoomLevel = newScale;
    ...
    function lineAddEvents() {
      drawingLayer.getChildren().forEach((line) => {
        if (line.className === 'Line' && !line._hasLineEvents) {
          ...
          line.on('mouseout', () => { stage.container().style.cursor = isDrawingMode ? 'crosshair' : 'default'; });
        }
      });
    }
      ...
      Konva.Image.fromURL(iconPathIdle, function (icon) {
        ...
        function handleInteraction(e) {
          ...
          if (icon.getAttr('sound')) {
            ...
            if (typeof cfg.showToast === 'function') cfg.showToast('Not found the sound id!');
          }
        }
        ...
        icon.on('mouseout', function () { stage.container().style.cursor = isDrawingMode ? 'crosshair' : 'default'; });
    ...
    function getSoundStartEnd(fileName) {
      if (typeof cfg.getSoundStartEnd === 'function') return cfg.getSoundStartEnd(fileName);
      if (!fileName) return [];
    ...
    function loadJsonBackgroundAndIcons(page, data) {
      ...
      imageObj.src = (cfg.global_const && cfg.global_const.PATH_ASSETS_IMG ? cfg.global_const.PATH_ASSETS_IMG : '') + data.background;
    }
    ...
    function setupPointerHandlers() {
      ...
      function cancelPendingDraw() { if (touchDrawTimer) { clearTimeout(touchDrawTimer); touchDrawTimer = null; } }
      function cancelActiveDrawing() { if (isDrawing) { isDrawing = false; lastLine = null; } }
  ...
  if (native && typeof native.clientX !== 'undefined') {
    ...
    zoomAtStagePointer(zoomStep);
      ...
      container.addEventListener('pointermove', function (evt) {
        ...
        if (swipeState.active) {
          ...
          if (Math.abs(dy) > SWIPE_MAX_VERTICAL) {
            ...
            if (!swipeState.fired && Math.abs(dx) >= SWIPE_THRESHOLD && (Date.now() - lastSwipeTime > SWIPE_COOLDOWN)) {
              ...
              if (typeof cfg.onPageChangeRequest === 'function') cfg.onPageChangeRequest(isNext);
        ...
        // drawing continue
        if (pinchState.isPinching) { cancelPendingDraw(); cancelActiveDrawing(); return; }
        if (!isDrawing) return;
        const pt = clientToStage(evt.clientX, evt.clientY);
        if (!lastLine) return;
      ...
      function releasePointer(evt) {
        ...
        if (swipeState.active) {
          swipeState.active = false; swipeState.fired = false;
        }
    ...
    function loadLinesByDraw(page, rawLinesArray, tries = 0) {
      ...
      if (!backgroundImage || !backgroundImage.image()) {
        if (tries < 5) setTimeout(() => loadLinesByDraw(page, rawLinesArray, tries + 1), 60);
        else console.warn('backgroundImage not ready for loadLinesByDraw', page);
        return;
      }
    ...
    function exportDrawnLines() {
      ...
      const bgDisplay = { x: backgroundImage.x(), y: backgroundImage.y(), width: backgroundImage.width(), height: backgroundImage.height() };
      ...
      return {
        lines: drawnLines,
        meta: {
          savedAtDisplay: { x: bgDisplay.x, y: bgDisplay.y, width: bgDisplay.width, height: bgDisplay.height },
    ...
    function undoLastLine() {
      ...
      try { last.destroy(); } catch (e) { /* ignore */ }
      drawingLayer.batchDraw();
      return true;
    }
    ...
    return {
      ...
      clearCanvas,
      deleteSelectedLine,
      setDrawingMode: function (flag) { isDrawingMode = !!flag; stage.container().style.cursor = isDrawingMode ? 'crosshair' : 'default'; },
      toggleDrawing: function () { isDrawingMode = !isDrawingMode; this.setDrawingMode(isDrawingMode); },
      processNextPrePage,
      getState: function () {
        return { stage, backgroundLayer, iconLayer, drawingLayer, backgroundImage, playIcons, currentIcon, lines, isDrawingMode };
      },
       // <-- thêm exports mới
      zoomIn,
      zoomOut,
