...
$(document).ready(function () {
      ...
      if (isDrawing) {
        ...
        if (
          ...
          document
            ...
            .classList.contains("bi-unlock-fill")
        ) {
          stage.draggable(true);
        } else {
          stage.draggable(false);
  ...
  function loadLinesByDraw(page, _tryCount = 0) {
    ...
    if (!backgroundImage || !backgroundImage.image()) {
      if (_tryCount < 5) {
        ...
        console.warn(
          "backgroundImage not ready after retries when loading lines for page",
          page
        );
      }
      return;
    ...
    const natW =
      imgEl && (imgEl.naturalWidth || imgEl.width)
        ? imgEl.naturalWidth || imgEl.width
        : bgW;
    const natH =
      imgEl && (imgEl.naturalHeight || imgEl.height)
    ...
    const coordSystem =
      rawEntry && rawEntry.meta && rawEntry.meta.coordSystem
        ? rawEntry.meta.coordSystem
        : null;
  ...
  function sendJsonToServer() {
    ...
    const naturalWidth =
      imgNode && (imgNode.naturalWidth || imgNode.width)
        ? imgNode.naturalWidth || imgNode.width
        : bgDisplay.width;
    const naturalHeight =
      imgNode && (imgNode.naturalHeight || imgNode.height)
        ? imgNode.naturalHeight || imgNode.height
        : bgDisplay.height;
